...
public class Ex01_BookTest {
	...
	public void equalsTest() {
		Book b1 = new Book("댄브라운", "천사와 악마", 25000);
		//assertEquals(b1.author, "무라카미 하루키");  
		assertEquals(b1.author, "댄브라운");  
	...
	public void sameTest() {
		Book b1 = new Book("댄브라운", "천사와 악마", 25000);
		Book b2 = new Book("무라카미 하루키", "상실의 시대", 30000);  
		Book b3 = new Book("무라카미 하루키", "상실의 시대", 30000);  
		//assertSame(b1, b2); //false
		//assertSame(b2, b3); //false -> 값이 같아 보이지만 메모리상의 주소가 다르므로 다른 객체다.
		b1 = b3;
	...
	public void nullTest() {
		Book b1; 
		// => 인스턴스를 정의만 하고 생성하지않은 상태
		//    지역변수는 초기화 하지 않으며 오류, 사용시 오류발생
		Book b2 = new Book("무라카미 하루키", "상실의 시대", 30000); //인스턴스 생성
		b2 = null; //인스턴스 삭제 
		assertNotNull(b2);
	}
	@Test
	...
	public void arrayEqualsTest() {
		...
		//assertArrayEquals(a1, a2); //true -> Equals = 값 비교
		...
		// => 각 배열의 Data는 3개씩
		Book b1 = new Book("오다 에이치로", "원피스", 7000);
		Book b2 = new Book("우라사와 나오키", "20세기 소년", 7000);  
		Book b3 = new Book("토리야마 아키라", "드래곤볼", 6000); 
		Book[] ba1 = {b1, b2, b3};
		Book[] ba2 = {b2, b1, new Book("이노우에 타케히코", "슬램덩크", 7000)};
		Book[] ba3 = {b1, b2, b3};
